...
public class LijurServiceClient extends ServiceClient {
    public final static String URL_LOGON = "https://192.168.4.10/www/pfe/webservice.php?q=LIJUR";

    protected String userName;
    protected String token;

    public LijurServiceClient(Context context, String user, String token) {
        super(context);
        this.userName = user;
        this.token = token;
    }

    public LijurServiceClient(Context context, User user) {
    ...
    protected void onPostExecute(JsonObject response) {
        ...
        if (result.equals("OK")) {
            this.result = new Gson().fromJson(response.toString(), LijurServiceResult.class);
        } else {
