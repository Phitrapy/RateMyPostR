...
public class MyprjServiceClient extends ServiceClient {

    public final static String URL_LOGON = "https://192.168.4.10/www/pfe/webservice.php?q=MYPRJ";

    private String userName;
    private String token;

    public MyprjServiceClient(Context context, String user, String token) {
        super(context);
        this.userName = user;
        this.token = token;
    }

    public MyprjServiceClient(Context context, User user) {
    ...
    protected void onPostExecute(JsonObject response) {
        ...
        if (result.equals("OK")) {
            ...
            Type projectArrayType = new TypeToken<ArrayList<Projet>>(){}.getType();
            projets = new Gson().fromJson(response.get("projects").toString(), projectArrayType);
            this.result = new MyprjServiceResult(result, projets);
        } else {
